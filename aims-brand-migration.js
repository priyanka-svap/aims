...
const masterBrands = [
  { name: 'Mc Dowells',        cat: 'whisky' },
  { name: 'Green Lable',       cat: 'whisky' },
  { name: 'Royal Stage',       cat: 'whisky' },
  { name: 'Imperial Blue',     cat: 'whisky' },
  { name: 'Royal Challenge',   cat: 'whisky' },
  { name: 'All Season',        cat: 'whisky' },
  { name: 'Officer Choice',    cat: 'whisky' },
  { name: 'Blender Pride',     cat: 'whisky' },
  { name: 'Signature Premium', cat: 'whisky' },
  { name: 'Signature Rare',    cat: 'whisky' },
  { name: 'Teacher 50',        cat: 'whisky' },
  { name: 'Black Dog Black',   cat: 'whisky' },
  { name: '100 Piper',         cat: 'whisky' },
  { name: 'Signature Prem',    cat: 'whisky' },
  { name: 'Rock Ford',         cat: 'whisky' },
  { name: 'Kingfisher Beer',   cat: 'beer'   },
  { name: 'Carls Berg',        cat: 'beer'   },
  { name: 'MM Orange',         cat: 'other'  },
  { name: 'RS Pav',            cat: 'desi'   },
  { name: 'No 01 Pav',         cat: 'desi'   },
  { name: 'GL Pav',            cat: 'desi'   },
  { name: 'As Pav',            cat: 'desi'   },
  { name: 'Oc Pav',            cat: 'desi'   },
];
...
async function migrate() {
  ...
  // ── 2. MasterBrands (brand tab mein dikhne ke liye) ───────────────────────
  console.log(`MasterBrands: ${masterBrands.length} records migrate honge`);
  const mbStats = { inserted: 0, skipped: 0, errors: 0 };

  for (const mb of masterBrands) {
    if (DRY_RUN) {
      console.log(`  [DRY] Upsert MasterBrand: ${mb.name.padEnd(22)} cat=${mb.cat}`);
      mbStats.inserted++;
      continue;
    }
    try {
      await MasterBrand.findOneAndUpdate(
        { name: mb.name },
        { $setOnInsert: mb },  // existing records ko touch mat karo
  ...
  console.log(`  ✓ Inserted/Upserted: ${mbStats.inserted}  Errors: ${mbStats.errors}\n`);
